...
const SCRIPTED_RESPONSES = {
  ...
  'compare lums and iba': {
    ...
    data: {
      institutions: [
        { name: 'LUMS', rank: '#03', placement: '96.1%', strength: 'Business & CS', fees: 'PKR 420,000/Sem', rating: 4.8 },
        { name: 'IBA', rank: '#05', placement: '94.8%', strength: 'Finance & MBA', fees: 'PKR 310,000/Sem', rating: 4.7 },
      ],
    },
  },
  'who graduated from fast': {
    ...
    data: {
      alumni: [
        { name: 'Osman Butt', role: 'VP of Engineering', company: 'Careem / Uber', badge: 'Platform Architecture' },
        { name: 'Nimra Saeed', role: 'Principal Security Architect', company: 'Cloudflare', badge: 'Cyber Security' },
        { name: 'Bilal Ahmed', role: 'Staff Systems Engineer', company: 'Stripe', badge: 'Distributed Ledgers' },
      ],
    },
  },
  'upcoming events': {
    ...
    data: {
      events: [
        { title: 'Pakistan Tech Innovation Summit', date: 'Oct 15, 2026', venue: 'NUST H-12', category: 'Conference' },
        { title: 'National Hackathon: Pakistan 2.0', date: 'Dec 10, 2026', venue: 'SEECS, NUST', category: 'Hackathon' },
        { title: 'LUMS Startup Village Pitch Day', date: 'Nov 12, 2026', venue: 'NIC Lahore', category: 'Entrepreneurship' },
      ],
    },
  },
  'what can eduhub do': {
    ...
    data: {
      tiers: [
        { role: 'Super Admin', desc: 'Global tenant provisioning & audit trails' },
        { role: 'Institute Admin', desc: 'Multi-branch enrollment & fee analytics' },
        { role: 'Campus Manager', desc: 'Timetables, exams & local admissions' },
        { role: 'Faculty', desc: 'Gradebooks, diaries & assignment grading' },
        { role: 'Students & Parents', desc: 'Grades, attendance & fee vouchers' },
      ],
    },
  },
};
  ...
  return (
    <div className="ai-response-card">
      ...
      <div className="ai-card-metrics">
        ...
        <div className="ai-metric">
          <CurrencyCircleDollar size={14} className="text-violet-500" weight="fill" />
          <span>{data.fees}</span>
        </div>
        <div className="ai-metric">
    ...
    <div className="ai-comparison-grid">
      {data.institutions.map((inst) => (
        <div key={inst.name} className="ai-response-card ai-comparison-item">
          ...
          <div className="ai-card-metrics">
            <div className="ai-metric"><Trophy size={13} className="text-amber-500" weight="fill" /><span>Rank {inst.rank}</span></div>
            <div className="ai-metric"><ChartBar size={13} className="text-emerald-500" weight="fill" /><span>{inst.placement}</span></div>
            <div className="ai-metric"><Sparkle size={13} className="text-blue-500" weight="fill" /><span>{inst.strength}</span></div>
            <div className="ai-metric"><CurrencyCircleDollar size={13} className="text-violet-500" weight="fill" /><span>{inst.fees}</span></div>
            <div className="ai-metric"><Star size={13} className="text-amber-400" weight="fill" /><span>{inst.rating} / 5.0</span></div>
          </div>
        </div>
      ))}
    </div>
      ...
      {data.alumni.map((a) => (
        <div key={a.name} className="ai-alumni-item">
          ...
          <div className="ai-alumni-info">
            ...
            <span className="ai-alumni-role">{a.role} @ {a.company}</span>
          </div>
      ...
      {data.events.map((ev) => (
        <div key={ev.title} className="ai-event-item">
          <div className="ai-event-date">
            <CalendarCheck size={16} className="text-emerald-500" weight="duotone" />
            <span>{ev.date}</span>
          </div>
          <h5 className="ai-event-title">{ev.title}</h5>
          <p className="ai-event-venue">{ev.venue} · {ev.category}</p>
        </div>
      ))}
    </div>
  );
...
function ResponseRenderer({ response }) {
  ...
  switch (response.type) {
    case 'institute': return <InstituteCard data={response.data} />;
    case 'comparison': return <ComparisonCard data={response.data} />;
    case 'alumni': return <AlumniCard data={response.data} />;
    case 'events': return <EventsCard data={response.data} />;
    case 'capabilities': return <CapabilitiesCard data={response.data} />;
    default: return null;
  }
}

const INTRO_TEXTS = {
  'tell me about nust': 'Here\'s the verified intelligence profile for NUST — Pakistan\'s top-ranked STEM institution:',
  'compare lums and iba': 'Here\'s a side-by-side comparison of LUMS and IBA based on verified institutional data:',
  'who graduated from fast': 'Here are notable FAST-NUCES alumni now working at top global technology companies:',
  'upcoming events': 'Here are the upcoming campus events across EduHub member institutions:',
  'what can eduhub do': 'EduHub operates across 5 governance tiers with strict role-scoped access control:',
};
...
export default function AIChatDemo() {
  ...
  const handleSend = (text) => {
    ...
    const matchKey = Object.keys(SCRIPTED_RESPONSES).find(key =>
    ...
    <section className="ai-chat-section" id="ai">
      ...
      >
        ...
        <h2 className="ai-section-title">
          ...
          <br />
          <span className="ai-title-bold text-gradient-emerald">Pakistan's institutions.</span>
        </h2>
        <p className="ai-section-subtitle">
          EduHub AI aggregates verified institutional data — rankings, fees, alumni placement, and campus events — into a single conversational interface.
        </p>
      </motion.div>
        ...
        <div className="ai-messages-area">
          ...
          {messages.length === 0 && !isTyping && (
            <div className="ai-welcome">
              ...
              <p>Ask me about institutions, rankings, alumni, events, or platform capabilities.</p>
            </div>
          )}
