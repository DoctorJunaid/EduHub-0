...
import { ArrowLeft, MessageCircle, Paperclip, Search, Send, SquarePen } from 'lucide-react';
import { Card } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { Textarea } from '@/components/ui/textarea';
import { Avatar, AvatarFallback } from '@/components/ui/avatar';
import { selectStudentConversations, searchStudentConversations, replyToStudentConversation } from '@/store/selectors/studentMessages';
...
function ConversationPanel({ conversation, onBack }) {
  ...
  useEffect(() => { if (history.current) history.current.scrollTop = history.current.scrollHeight; }, [conversation.messages.length]);
  const send = () => {
    if (!draft.trim()) return;
    const problem = dispatch(replyToStudentConversation({ conversationId: conversation.id, body: draft }));
  ...
  return <>
    ...
    <form className="sm-composer" onSubmit={(event) => { event.preventDefault(); send(); }}>
      <p className="sm-demo">Frontend demo: messages are saved in this browser, not delivered remotely.</p>
      {error && <p role="alert" className="sm-error">{error}</p>}
      <div className="sm-composer-field"><Button type="button" variant="ghost" size="icon" disabled aria-label="Attachments unavailable" title="File uploads are not supported"><Paperclip /></Button>
        <Textarea aria-label="Message" rows={1} placeholder="Type your message..." value={draft} disabled={!conversation.participant} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); send(); } }} />
        <Button type="submit" className="sm-send" size="icon" aria-label="Send message" disabled={!draft.trim() || !conversation.participant}><Send /></Button>
      </div>
    </form>
  </>;
}
