...
export default function AttendanceTable({ rows, view, page, pageSize, onAction }) {
  const current = Math.min(page, Math.max(1, Math.ceil(rows.length / pageSize)));
  const columns = view === 'weekly' ? ['#', 'Teacher / Staff', 'Department', ...attendanceStatuses] : ['#', 'Teacher / Staff', 'Department', ...(view === 'history' ? ['Date'] : []), 'Check-in Time', 'Check-out Time', 'Status', 'Actions'];
    ...
    {rows.slice((current - 1) * pageSize, current * pageSize).map(({ person, record, date, counts }, index) => <TableRow key={record?.id ?? person.id}>
      ...
      {view === 'weekly' ? attendanceStatuses.map((status) => <TableCell key={status}>{counts[status]}</TableCell>) : <>
        ...
        <TableCell><div className="attendance-actions"><Button variant="outline" onClick={() => onAction(record ? 'view' : 'edit', person.id, record?.id)} aria-label={`${record ? 'View' : 'Record'} attendance for ${person.name}`}>{record ? 'View' : 'Record'}</Button>
          {record && <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" aria-label={`Attendance actions for ${person.name}`}><MoreVertical size={15} /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => onAction('view', person.id, record.id)}>View details</DropdownMenuItem><DropdownMenuItem onSelect={() => onAction('edit', person.id, record.id)}>Update attendance</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}
        </div></TableCell>
      </>}
    </TableRow>)}
