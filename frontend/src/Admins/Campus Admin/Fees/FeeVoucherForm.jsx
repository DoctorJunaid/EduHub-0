...
export default function FeeVoucherForm({
  ...
  const submit = (event) => {
    ...
    if (!students.some((student) => student.id === payload.studentId))
      return setError("Select a current student.");
    ...
    >
      ...
      >
        <div className="tt-dialog-heading">
          <DialogTitle>
            {record ? "Edit Fee Voucher" : "Add Fee Voucher"}
          </DialogTitle>
        </div>
        <p id={`${id}-help`} className="fee-form-note">
          Record the voucher details and payment status. Paid vouchers require
        ...
        <form onSubmit={submit}>
          <div className="tt-form-grid">
            ...
            {[
              ["voucherNo", "Voucher Number", "text", true],
              ["feeCategory", "Fee Category", "text", true],
              ["semester", "Semester (if applicable)", "text", false],
              ["amount", "Amount (PKR)", "number", true],
              ...
              <div className="tt-field" key={key}>
                ...
                <Input
                  ...
                  required={required}
                  value={values[key]}
                  list={options[key] ? `${id}-${key}-options` : undefined}
            ...
            <div className="tt-field">
              <Label htmlFor={`${id}-status`}>Payment Status *</Label>
              <select
                id={`${id}-status`}
                required
          ...
          <div className="tt-dialog-actions">
            ...
            <Button type="submit">
              {record ? "Save Changes" : "Add Fee Voucher"}
            </Button>
          </div>
        </form>
      </DialogContent>
