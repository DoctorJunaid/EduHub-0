...
export default function ResultForm({
  ...
  const submit = (event) => {
    ...
    if (
      ...
      return setError("Select a current student and scheduled exam.");
    if (
      records.some(
        (item) =>
      ...
      return setError(
        "This student already has a result for that exam and academic period. Edit the existing result.",
      );
    ...
    >
      ...
      >
        <div className="tt-dialog-heading">
          <DialogTitle>
            {record ? "Edit Result" : "Record Exam Result"}
          </DialogTitle>
        </div>
        <p className="results-form-note" id={`${id}-help`}>
          Enter the awarded grade and GPA if known. They are not calculated from
          marks.
        </p>
        <form onSubmit={submit}>
          <div className="tt-form-grid">
            ...
            <div className="tt-field">
              <Label htmlFor={`${id}-exam`}>Exam / Subject *</Label>
              <select
                id={`${id}-exam`}
                required
                value={values.examId}
              ...
              >
                ...
                {exams.map((exam) => (
                  <option key={exam.id} value={exam.id}>
                    {exam.subject} — {exam.examType} — {exam.date} —{" "}
                    {exam.section}
                  </option>
                ))}
              </select>
            ...
            {[
              ...
              ["semester", "Semester", "text", true],
              ["courseCode", "Course Code", "text", false],
              ["score", "Score", "number", true],
              ["totalMarks", "Total Marks", "number", true],
              ["grade", "Awarded Grade", "text", false],
              ["gpa", "Awarded GPA", "number", false],
              ...
              <div className="tt-field" key={key}>
                ...
                <Input
                  ...
                  required={required}
                  list={type === "text" ? `${id}-${key}-options` : undefined}
                  value={values[key]}
                  onChange={(e) => change(key, e.target.value)}
                />
                {type === "text" && (
                  <datalist id={`${id}-${key}-options`}>
                    {[
                      ...new Set(
                        records.map((item) => item[key]).filter(Boolean),
                      ),
                    ].map((value) => (
            ...
            <div className="tt-field">
              <Label htmlFor={`${id}-remarks`}>Academic Remarks</Label>
              <Textarea
                id={`${id}-remarks`}
          ...
          <div className="tt-dialog-actions">
            ...
            <Button type="submit">
              {record ? "Save Changes" : "Save Result"}
            </Button>
          </div>
        </form>
      </DialogContent>
