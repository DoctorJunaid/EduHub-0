...
export function validateResult(record) {
  for (const key of ["studentId", "examId", "academicYear", "semester"])
    if (typeof record[key] !== "string" || !record[key].trim())
      return "Select a student and exam, and enter an academic year and semester.";
  if (
    !Number.isFinite(record.score) ||
    record.score < 0 ||
    !Number.isFinite(record.totalMarks) ||
    record.totalMarks <= 0 ||
    record.score > record.totalMarks
  )
    return "Score must be between zero and the positive total marks.";
  if (record.gpa !== null && (!Number.isFinite(record.gpa) || record.gpa < 0))
    return "Enter a non-negative GPA or leave it blank.";
  if (
    ["grade", "courseCode", "remarks"].some(
      (key) => typeof record[key] !== "string",
    )
  )
    return "Invalid result text fields.";
  return "";
}
...
export function filterResults(rows, filters) {
  ...
  return rows.filter(
    (row) =>
      (!query ||
        [row.student.name, row.student.roll, row.exam.subject].some((value) =>
          value.toLowerCase().includes(query),
        )) &&
      (!filters.academicYear || row.academicYear === filters.academicYear) &&
      (!filters.semester || row.semester === filters.semester) &&
      (!filters.course || row.exam.subject === filters.course) &&
      (!filters.grade || row.grade === filters.grade) &&
      (!filters.gpa || (row.gpa !== null && String(row.gpa) === filters.gpa)) &&
...
export function resultsAnalytics(rows) {
  ...
  for (const row of rows) {
    ...
    if (!periods.has(period))
      periods.set(period, {
        label: `${row.semester} · ${row.academicYear}`,
        date: row.exam.date,
        rows: [],
      });
    const group = periods.get(period);
    group.rows.push(row);
    if (row.exam.date < group.date) group.date = row.exam.date;
    if (row.grade.trim())
      grades.set(row.grade, (grades.get(row.grade) ?? 0) + 1);
  }
  return {
    average: averageGpa(rows),
    students: new Set(rows.map((row) => row.studentId)).size,
    trend: [...periods.values()]
      .sort(
        (a, b) =>
          a.date.localeCompare(b.date) || a.label.localeCompare(b.label),
      )
      .map((group) => ({ label: group.label, value: averageGpa(group.rows) }))
