...
for (const [key, add, update, remove, edit] of [
  [
    ...
    campusUpdated,
    campusDeleted,
    { name: "Renamed Campus", address: "New address", status: "Active" },
  ],
  [
    "exams",
    addExam,
  ...
  test(`${key}: add, edit, delete survive store recreation (refresh)`, () => {
    ...
    for (const [field, value] of Object.entries(edit))
      assert.deepEqual(
        store
          .getState()
          [key].records.find((record) => record.id === created.id)[field],
        value,
      );
    store.dispatch(remove(created.id));
    store = create(storage);
    assert.equal(
      store.getState()[key].records.some((record) => record.id === created.id),
      false,
    );
    assert.equal(
      store.getState()[key].records.some((record) => record.id === original.id),
      true,
    );
  });
}
...
test("empty collections stay empty after refresh; seed records are not resurrected", () => {
  ...
  for (const key of Object.keys(storageKeys).filter((key) => refreshed.getState()[key]))
    assert.deepEqual(refreshed.getState()[key].records, []);
});
...
test("invalid JSON, wrong versions, invalid records, and duplicate IDs safely fall back", () => {
  ...
  for (const invalid of [
    ...
    JSON.stringify({ version: 1, records: [seed, seed] }),
    JSON.stringify({ version: 1, records: [{ ...seed, status: "Unknown" }] }),
  ]) {
    const storage = memory();
    storage.setItem(storageKeys.students, invalid);
    storage.setItem(
      storageKeys.faculty,
      JSON.stringify({ version: 1, records: [] }),
    );
...
test("invalid schedule timing or weekdays cannot crash grid hydration", () => {
  ...
  for (const patch of [
    ...
    storage.setItem(
      ...
      JSON.stringify({ version: 1, records: [{ ...seed, ...patch }] }),
    );
...
test("storage access/quota failures do not stop Redux; only changed collections are written", () => {
  ...
  assert.doesNotThrow(() =>
    store.dispatch(studentDeleted(store.getState().students.records[0].id)),
  );
  const storage = memory();
  const writes = [];
  ...
  const healthy = create(storage);
  writes.length = 0;
  healthy.dispatch({ type: "unrelated-ui-event" });
  assert.deepEqual(writes, []);
  healthy.dispatch(studentDeleted(healthy.getState().students.records[0].id));
  assert.deepEqual(writes, [storageKeys.students]);
  for (const value of storage.data.values())
    assert.deepEqual(Object.keys(JSON.parse(value)), ["version", "records"]);
});
