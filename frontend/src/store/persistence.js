...
export const demoLifecycleStorageKey = 'eduhub_demo_lifecycle';
const fields = {
  fees: [
    "studentId",
    "voucherNo",
...
export const storageKeys = {
  diary: "eduhub_diary",
  assignments: "eduhub_assignments",
  submissions: "eduhub_submissions",
  broadcasts: "eduhub_broadcasts",
  campuses: "eduhub_campuses",
  messages: "eduhub_messages",
  faculty: "eduhub_faculty",
  students: "eduhub_students",
  timetable: "eduhub_timetable",
  exams: "eduhub_exams",
  attendance: "eduhub_attendance",
  studentAttendance: "eduhub_student_attendance",
  results: "eduhub_results",
  fees: "eduhub_fees",
};
const statuses = {
  faculty: ["Active", "Pending", "Inactive"],
  students: ["Active", "Pending", "Graduated", "Suspended"],
  timetable: ["Active", "Pending"],
};

function validRecords(collection, records) {
  if (collection === "diary") return validDiaryRecords(records);
  if (collection === "assignments" || collection === "submissions")
    return validAssignmentRecords(records, collection === "submissions");
  ...
  return records.every((record) => {
    if (
      ...
      return false;
    ids.add(record.id);
    if (!fields[collection].every((field) => typeof record[field] === "string"))
      return false;
    if (
      collection === "students" &&
      ["campusId", "instituteId"].some(
        (field) =>
    ...
    if (collection === "fees")
      return (
        !validateVoucher(record) &&
        [record.createdAt, record.updatedAt].every((value) =>
    ...
    if (collection === "studentAttendance") {
      ...
      if (attendanceDays.has(key) || !validStudentAttendance(record))
        return false;
      attendanceDays.add(key);
      return true;
    }
    ...
    if (collection === "exams")
      return typeof record.totalMarks === "number" && !validateExam(record);
    ...
    if (collection === "timetable") {
      ...
      return (
        ...
        time.test(record.startTime) &&
        time.test(record.endTime) &&
...
export function loadDemoState(storage = browserStorage()) {
  const state = {};
  try {
    if (storage?.getItem(demoLifecycleStorageKey) || Object.values(storageKeys).some(key => storage?.getItem(key) != null))
      state.demoLifecycle = { initialized: true };
  } catch { /* Storage failure must not prevent rendering. */ }
  try {
    const saved = JSON.parse(storage?.getItem(authStorageKey) ?? "null");
    if (saved !== null)
      state.auth = sessionState(saved?.version === 1 ? saved.user : null);
  } catch {
    state.auth = sessionState(null);
  }
  for (const [collection, key] of Object.entries(storageKeys)) {
    try {
      const stored = JSON.parse(storage?.getItem(key) ?? "null");
      if (stored?.version === 1 && validRecords(collection, stored.records))
        state[collection] = { records: stored.records };
    } catch {
      /* Invalid or inaccessible storage falls back to the reducer's seed state. */
    }
  }
  return state;
}
...
export function persistDemoState(store, storage = browserStorage()) {
  ...
  const sync = () => {
    ...
    if (state.demoLifecycle && previous.demoLifecycle !== state.demoLifecycle) {
      try { storage?.setItem(demoLifecycleStorageKey, JSON.stringify({ version: 1, initialized: true })); previous.demoLifecycle = state.demoLifecycle; }
      catch { /* Keep in-memory state usable. */ }
    }
      ...
      try {
        if (state.auth.isAuthenticated)
          storage?.setItem(
            ...
            JSON.stringify({
              version: 1,
              user: sessionState(state.auth.user).user,
            }),
          );
        else storage?.removeItem(authStorageKey);
        previous.auth = state.auth;
      } catch {
        /* Keep in-memory auth usable if browser storage is unavailable. */
      }
    }
    for (const [collection, key] of Object.entries(storageKeys)) {
      ...
      try {
        ...
        /* Keep Redux usable when storage is full or unavailable; retry on a later change. */
      }
    }
  };
  sync();
