...
import { selectStudentDiary, filterDiaryBySubject, diaryEntriesForDate } from './studentDiary.js';
import { loadDemoState, persistDemoState } from '../persistence.js';

const reducer = { auth, students, timetable, faculty, studentAttendance, results, assignments, submissions, diary };
const entry = { id: 'note-1', classId: 'schedule-1', title: 'Lecture topic', date: '2026-09-10', recap: 'Lecture recap', homework: 'Practice notes', resources: 'Reading notes' };
const create = (records = [entry], tasks = []) => {
  const store = configureStore({ reducer, preloadedState: { diary: { records }, assignments: { records: tasks } } });
...
test('diary uses enrolled classes and section, excludes orphan entries and unlinked accounts', () => {
  const store = create([entry, { ...entry, id: 'other', classId: 'schedule-2' }, { ...entry, id: 'orphan', classId: 'missing' }]);
...
test('filter resets to all without mutation, dates sort newest first and today uses the same entries', () => {
  ...
  assert.deepEqual(rows.map((row) => row.id), ['note-1', 'old']);
  ...
  assert.deepEqual(store.getState().diary.records.map((row) => row.id), ['old', 'note-1']);
});
test('instructor and assignment references resolve from shared data, never from copied records', () => {
  const task = { id: 'task-1', classId: 'schedule-1', title: 'Shared assignment', dueDate: '2026-09-12', totalMarks: 20 };
  const store = create([{ ...entry, assignmentId: 'task-1' }], [task]);
  store.dispatch(classUpdated({ id: 'schedule-1', instructor: 'Updated Instructor' }));
  ...
  assert.equal(selectStudentDiary(create([{ ...entry, assignmentId: 'missing' }]).getState())[0].assignment, undefined);
  assert.equal(selectStudentDiary(create([{ ...entry, assignmentId: 'task-1' }], [{ ...task, classId: 'schedule-2' }]).getState())[0].assignment, undefined);
});
test('centralized persistence retains diary and rejects invalid dates, duplicates and malformed content', () => {
  ...
  assert.deepEqual(selectStudentDiary(restored.getState()), selectStudentDiary(store.getState()));
  ...
  assert.equal(validDiaryRecords([{ id: 'minimal', classId: 'schedule-1', date: '2026-09-10', title: 'Topic' }]), true);
  unsubscribe();
});
