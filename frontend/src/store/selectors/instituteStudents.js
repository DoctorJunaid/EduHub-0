...
export function joinInstituteStudents(students, campuses) {
  return students.flatMap((student) => {
    if (student.instituteId && student.instituteId !== demoInstitute.id)
      return [];
    const campusId =
      student.campusId ||
      initialCampuses.find((campus) => campus.name === student.campus)?.id ||
      campuses.find(
        (campus) =>
          campus.name === student.campus &&
          campus.instituteId === demoInstitute.id,
      )?.id;
    ...
    if (
      !student.instituteId &&
      !campus &&
      !initialCampuses.some((record) => record.id === campusId)
    )
      return [];
    return [
      {
        ...student,
        campusId: campusId || "",
        campus: campus?.name || "Campus unavailable",
      },
    ];
  });
}
