...
const reducer = { auth, students, timetable, studentAttendance, faculty, results };
...
test('course cards use dashboard enrollments and never borrow another section or guess subject aliases', () => {
  ...
  assert.equal(cards.length, selectStudentDashboard(store.getState()).courses.length);
  assert.deepEqual(cards.map((card) => card.title), ['Advanced Web Design', 'Data Structures', 'AI']);
  ...
  store.dispatch(studentUpdated({ id: 'student-demo-1', subjects: 'Data Structures & Algorithms', section: 'CS-4A' }));
  ...
  assert.equal(selectStudentCourses(store.getState())[0].routines[0].room, 'Hall B');
...
test('schedule updates, faculty matches and per-course personal attendance stay live', () => {
  const store = create();
  store.dispatch(facultyUpdated({ id: 'faculty-demo-1', name: 'Dr. Updated Teacher' }));
  store.dispatch(classUpdated({ id: 'schedule-1', instructor: 'dr. updated teacher', room: 'Lab 500', startTime: '11:00', endTime: '12:30' }));
  store.dispatch(studentAttendanceMarked({ studentId: 'student-demo-1', classId: 'schedule-1', date: '2026-09-07', status: 'Present' }));
  store.dispatch(studentAttendanceMarked({ studentId: 'student-demo-1', classId: 'schedule-1', date: '2026-09-09', status: 'Absent' }));
  store.dispatch(studentAttendanceMarked({ studentId: 'student-demo-2', classId: 'schedule-1', date: '2026-09-09', status: 'Present' }));
  store.dispatch(studentAttendanceMarked({ studentId: 'student-demo-1', classId: 'schedule-2', date: '2026-09-08', status: 'Present' }));
...
test('invalid schedule fields yield a clean fallback instead of malformed punctuation', () => {
  ...
  for (const patch of [{ days: [] }, { days: [9] }, { startTime: '' }, { endTime: 'bad' }, { endTime: '09:00' }]) assert.equal(courseScheduleLabel({ ...valid, ...patch }), 'Schedule not available');
});
...
test('empty/unlinked enrollments and refreshed shared records remain synchronized', () => {
  ...
  store.dispatch(demoLoggedIn({ role: 'student', email: 'unlinked@example.com' }));
