...
const save = (store, extra = {}) =>
  store.dispatch(
    resultSaved({
      ...
      examId: store.getState().exams.records[0].id,
...
test("results are personal, joined to actual exams and grouped by academic year and semester", () => {
  const store = create();
  save(store);
  save(store, { studentId: "student-demo-2" });
  save(store, { examId: "missing" });
  save(store, { semester: "Fall 2026", academicYear: "2026 - 2027" });
...
test("CGPA stays synchronized without averaging exam awards; shared result corrections remain live", () => {
  ...
  assert.equal(
    selectStudentGrades(store.getState()).cgpa,
    selectStudentDashboard(store.getState()).cgpa,
  );
  save(store, { score: 92, grade: "A+", gpa: 4, remarks: "Recorded feedback" });
  assert.equal(
    selectStudentGrades(store.getState()).periods[0].rows[0].remarks,
    "Recorded feedback",
  );
...
test("empty/unlinked accounts expose no awards; centralized persistence restores results", () => {
  ...
  assert.equal(
    selectStudentGrades(restored.getState()).periods[0].rows[0].score,
    0,
  );
  store.dispatch(
