...
const normalizeResult = (item) => {
  ...
  const examId =
    item.examId ||
    item.examName ||
    `exam-${String(item.subject || "sub").slice(0, 4)}`;
  const courseCode = item.courseCode || item.subject || "General";
  const grade =
    item.grade !== undefined
      ? item.grade
      : score / totalMarks >= 0.8
        ? "A"
        : "B";
  const gpa =
    item.gpa !== undefined ? item.gpa : Number((score / totalMarks) * 4);
  const remarks =
    item.remarks !== undefined ? item.remarks : "Satisfactory performance";
...
export const fetchResults = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get("/campus-admin/performance", {
        params,
      });
    ...
    try {
      const payload = {
        ...
        marksObtained: Number(
          resultData.score ?? resultData.marksObtained ?? 0,
        ),
        totalMarks: Number(resultData.totalMarks || 100),
        examName:
          resultData.examName || resultData.semester || "Midterm Examination",
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to save exam result",
      );
    }
  },
);
...
export const { resultsLoaded, resultSaved } = slice.actions;
