  ...
  reducers: {
    studentsLoaded: (state, { payload }) => {
      ...
      const normalized = {
        ...
        initials: student.initials || initialsFor(student.name || ""),
      };
      const index = state.records.findIndex((row) => row.id === normalized.id);
      if (index === -1) state.records.push(normalized);
      else state.records[index] = { ...state.records[index], ...normalized };
    ...
    builder
      ...
      .addCase(fetchStudents.fulfilled, (state, { payload }) => {
        ...
        state.records = list.map((student) => ({
          ...
          initials:
            student.initials || initialsFor(student.name || "Unknown Student"),
        }));
      })
      ...
      .addCase(addStudent.fulfilled, (state, { payload }) => {
        const newStudent = {
          ...
          initials:
            payload.initials || initialsFor(payload.name || "Unknown Student"),
        };
      ...
      .addCase(updateStudent.fulfilled, (state, { payload }) => {
        ...
        if (index !== -1) {
          state.records[index] = {
            ...
            initials:
              payload.initials ||
              initialsFor(payload.name || "Unknown Student"),
          };
        }
      })
      // Delete
      .addCase(deleteStudent.fulfilled, (state, { payload }) => {
        state.records = state.records.filter(
          (student) => student.id !== payload && student._id !== payload,
        );
      });
  },
});

export const { studentsLoaded, studentAdded, studentUpdated, studentDeleted } =
