...
const facultySlice = createSlice({
  ...
  reducers: {
    facultyLoaded: (state, { payload }) => {
      state.records = (Array.isArray(payload) ? payload : []).map(
        (faculty) => ({
          ...faculty,
          id: faculty.id || faculty._id,
          campus: faculty.campus || faculty.campusId?.name || "",
          initials:
            faculty.initials || initialsFor(faculty.name || "Unknown Faculty"),
        })
      );
      ...
      .addCase(updateFaculty.fulfilled, (state, { payload }) => {
        ...
        if (index !== -1) {
          state.records[index] = {
            ...
            initials:
              payload.initials || initialsFor(payload.name || "Unknown"),
          };
        }
      })
      .addCase(deleteFaculty.fulfilled, (state, { payload }) => {
        state.records = state.records.filter(
          (faculty) => faculty.id !== payload && faculty._id !== payload
        );
      });
  },
});

export const { facultyLoaded, facultyAdded, facultyUpdated, facultyDeleted } =
