  ...
  reducers: {
    ...
    messageSent: {
      prepare: ({ participantId, participantType, body }) => ({ payload: {
        ...
        id: nanoid(), createdAt: new Date().toISOString(),
      } }),
      reducer: (state, { payload }) => {
        if (!payload.body || !payload.participantId || !['student', 'faculty'].includes(payload.participantType)) return;
        const conversationId = conversationIdFor(payload.participantType, payload.participantId);
        let conversation = state.records.find((record) => record.id === conversationId);
        if (!conversation) {
          conversation = { id: conversationId, participantId: payload.participantId, participantType: payload.participantType, messages: [], updatedAt: payload.createdAt };
          state.records.push(conversation);
          conversation = state.records.at(-1);
        }
        conversation.messages.push({ id: payload.id, conversationId, senderId: LOCAL_SENDER_ID, receiverId: payload.participantId, body: payload.body, createdAt: payload.createdAt });
...
export const selectConversations = createSelector(
  [(state) => state.students.records, (state) => state.faculty.records, (state) => state.messages.records],
  (students, faculty, records) => [
    ...faculty.map((participant) => ({ participant, participantType: 'faculty' })),
    ...students.map((participant) => ({ participant, participantType: 'student' })),
    ...
    return { id, participantId: participant.id, participantType, participant, messages, lastMessage: messages.at(-1)?.body ?? '', updatedAt: saved?.updatedAt ?? null };
  }).sort((a, b) => (Date.parse(b.updatedAt) || 0) - (Date.parse(a.updatedAt) || 0)),
);

export function filterConversations(conversations, search) {
  const query = search.trim().toLowerCase();
  return conversations.filter(({ participant, lastMessage }) => `${participant.name} ${participant.email} ${lastMessage}`.toLowerCase().includes(query));
}
...
export function validConversations(records) {
  ...
  return records.every((record) => {
    ...
    return record.messages.every((message) => {
      if (!message || typeof message.id !== 'string' || !message.id || messageIds.has(message.id) || message.conversationId !== record.id || typeof message.body !== 'string' || !message.body.trim() || typeof message.createdAt !== 'string' || !Number.isFinite(Date.parse(message.createdAt))) return false;
      if (!((message.senderId === LOCAL_SENDER_ID && message.receiverId === record.participantId) || (message.senderId === record.participantId && message.receiverId === LOCAL_SENDER_ID))) return false;
      const time = Date.parse(message.createdAt);
      if (time < previousTime || time > Date.parse(record.updatedAt)) return false;
      previousTime = time;
